/* eslint-disable max-len, implicit-arrow-linebreak, no-confusing-arrow, function-paren-newline, semi */
const defaultBehavior = <T>(promise: Promise<Response>) =>
  promise.then((res) =>
    res.ok ? (res.json() as Promise<T>) : res.json().then((error) => Promise.reject(error)),
  )

const caller = <T>(url: string, query = {}) =>
  defaultBehavior<T>(fetch(`/api/${url}?${new URLSearchParams(query).toString()}`))
