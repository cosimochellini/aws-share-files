...
export type ServiceMapper<T extends Record<string, GenericFunction> | ServiceMapper<T>> = {
  [key in keyof T]: T[key] extends GenericFunction ? ResponseType<T[key]> : T[key]
}

export type AwaitedServiceMapper<T extends Record<string, GenericFunction> | ServiceMapper<T>> = {
  [key in keyof T]: T[key] extends GenericFunction
    ? Awaited<ResponseType<T[key]>>
    : /* @ts-expect-error: recursive type reference */
      AwaitedServiceMapper<T[key]>
}

export type ServiceArguments<T extends Record<string, GenericFunction>> = {
  [key in keyof T]: T[key] extends GenericFunction ? Parameters<T[key]>[0] : never
}
